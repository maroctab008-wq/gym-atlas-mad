...
export default function AuditLogs() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-semibold text-foreground">Journal d'Audit</h1><p className="text-muted-foreground text-sm mt-1">Historique des actions des utilisateurs</p></div>
      {logs.length === 0 ? (
        <Card className="shadow-sm"><CardContent className="p-12 text-center"><ClipboardList className="w-12 h-12 mx-auto text-muted-foreground mb-3" /><p className="text-muted-foreground">Aucun log d'audit pour le moment</p></CardContent></Card>
      ) : (
        <Card className="shadow-sm overflow-hidden"><CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {logs.map((log) => (
                <TableRow key={log.id}>
                  <TableCell className="text-sm text-muted-foreground font-mono">{formatDateFR(log.created_at)} {formatTimeFR(log.created_at)}</TableCell>
                  <TableCell className="text-sm font-medium">{log.user_name || log.user_email || '—'}</TableCell>
                  ...
                  <TableCell className="text-xs text-muted-foreground font-mono max-w-48 truncate">{log.details ? JSON.stringify(log.details) : '—'}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
