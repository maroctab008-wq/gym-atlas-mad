...
import { Settings as SettingsIcon, Save, Loader2, Building2, Shield } from 'lucide-react';
    ...
    const load = async () => {
      ...
      if (data) {
        for (const row of data) {
          ...
          if (row.key === 'gym_branding') {
            setBranding(v as BrandingData);
          }
          if (row.key === 'gate_control') {
            setGate(v as GateData);
          }
        }
      }
      setLoading(false);
          ...
          <Card className="shadow-sm max-w-xl">
            ...
            <CardContent className="space-y-6">
              ...
              <Button onClick={() => saveSection('access_rules', { block_after_days_late: blockAfterDays, allow_balance_due_entry: allowBalanceDue })} disabled={saving === 'access_rules'} className="gap-2">
                {saving === 'access_rules' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                Enregistrer
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm max-w-xl">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input value={branding.gym_name} onChange={e => setBranding({ ...branding, gym_name: e.target.value })} className="mt-1" placeholder="Ex: Atlas Gym" />
              </div>
              <div>
                <Label className="text-sm">Téléphone</Label>
                <Input value={branding.phone} onChange={e => setBranding({ ...branding, phone: e.target.value })} className="mt-1" placeholder="+212 600 000 000" />
              </div>
              <div>
                <Label className="text-sm">Site Web</Label>
                <Input value={branding.website} onChange={e => setBranding({ ...branding, website: e.target.value })} className="mt-1" placeholder="www.monsite.ma" />
              </div>
              <div>
                <Label className="text-sm">Adresse</Label>
                <Input value={branding.address} onChange={e => setBranding({ ...branding, address: e.target.value })} className="mt-1" placeholder="Casablanca, Maroc" />
              </div>
              <div>
                <Label className="text-sm">ICE (Identifiant Commun des Entreprises)</Label>
                <Input value={branding.ice} onChange={e => setBranding({ ...branding, ice: e.target.value })} className="mt-1" placeholder="Ex: 001234567000012" />
              </div>
              <div>
                <Label className="text-sm">URL du Logo</Label>
                <Input value={branding.logo_url} onChange={e => setBranding({ ...branding, logo_url: e.target.value })} className="mt-1" placeholder="https://..." />
              ...
              <Button onClick={() => saveSection('gym_branding', branding)} disabled={saving === 'gym_branding'} className="gap-2">
                {saving === 'gym_branding' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                Enregistrer
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm max-w-xl">
            ...
            <CardContent className="space-y-6">
              <div>
                ...
                <Input value={gate.controller_ip} onChange={e => setGate({ ...gate, controller_ip: e.target.value })} className="mt-1 font-mono" placeholder="192.168.1.100" />
              </div>
              <div>
                <Label className="text-sm">Clé API du contrôleur</Label>
                <Input type="password" value={gate.api_key} onChange={e => setGate({ ...gate, api_key: e.target.value })} className="mt-1 font-mono" placeholder="••••••••" />
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <Label className="text-sm">Application stricte du paiement</Label>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Refuser l'accès si le solde &gt; 0 MAD (même avec abonnement actif)
                  </p>
                </div>
              ...
              <div className="rounded-lg border border-border p-4 space-y-2 bg-secondary/30">
                ...
                <div className="text-xs space-y-1 text-muted-foreground">
                  <p className="flex items-center gap-2">
                    ...
                    <strong className="text-foreground">Accès accordé:</strong> Abonnement actif + Solde = 0 MAD → Relais 1
                  </p>
                  <p className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-destructive" />
                    <strong className="text-foreground">Accès refusé:</strong> Expiré ou Solde &gt; 0 MAD → Alarme rouge
                  </p>
                  <p className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-warning" />
                    <strong className="text-foreground">Override admin:</strong> Forcer l'ouverture ou Passe unique (admin uniquement)
                  </p>
                </div>
              </div>
